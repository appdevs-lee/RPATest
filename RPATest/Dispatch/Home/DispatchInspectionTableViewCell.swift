//
//  DispatchInspectionTableViewCell.swift
//  RPATest
//
...
protocol DispatchInspectionDelegate: NSObjectProtocol {
...
final class DispatchInspectionTableViewCell: UITableViewCell {
    ...
    lazy var normalButton: UIButton = {
        ...
        button.layer.cornerRadius = 15
        button.addTarget(self, action: #selector(tappedNormalButton(_:)), for: .touchUpInside)
        button.translatesAutoresizingMaskIntoConstraints = false
...
extension DispatchInspectionTableViewCell {
    // Set view foundation
...
extension DispatchInspectionTableViewCell {
    @objc func tappedFineButton(_ sender: UIButton) {
        self.status.fine = 1
        self.status.normal = 0
        self.status.notFine = 0
        
        self.delegate?.tappedFineButton(status: self.status)
    ...
    @objc func tappedNormalButton(_ sender: UIButton) {
        self.status.fine = 0
        self.status.normal = 1
        self.status.notFine = 0
        
        self.delegate?.tappedNormalButton(status: self.status)
    ...
    @objc func tappedNotFineButton(_ sender: UIButton) {
        self.status.fine = 0
        self.status.normal = 0
        self.status.notFine = 1
        
        self.delegate?.tappedNotFineButton(status: self.status)
